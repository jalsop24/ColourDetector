...
def combineColours(colourData):
    ...
    averageColoursList = np.zeros( (colourData.shape[0], 2), dtype=LAB_DTYPE ) # TODO dynamically update the size of this array rather than have it be as large as possible
    uniqueColours = 0
    ...
    for x in range( len(colourData[:,0]) ):
        ...
        if uniqueColour:
            ...
            # TODO insert the new colour in the correct place as we to remove the need for sorting
